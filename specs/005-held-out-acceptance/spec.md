...
- AC1: A runtime-neutral workflow doc defines the held-out check set: per-spec,
  ...
  maker-eval channels); each entry maps to a `US#` and is independently
  executable/verifiable by the acceptance [reviewer]; the doc names **[roles]** only
  (constitution P1).
- AC2: An **in-repo manifest** records a content hash of each spec's held-out set; a
  ...
  an error.
- AC3: The maker-role context **never observes held-out content before its gate
  outcome**: a deterministic mechanism denies maker-phase reads of the channel path,
  proven live by a falsification test that fires on a planted maker-phase read of the
  channel AND passes a control where the acceptance [reviewer]'s dispatch consumes the
  same content (enforcement is deterministic, not prompt discipline — constitution
  P2/P3); the enforcement change ships its matching test cases in the same diff, wiring
  assertion included (constitution P2). A test exercising only the denial direction does
  not satisfy this.
...
- AC1: The acceptance [reviewer]'s spec gains a held-out step: for the task's mapped
  `US#`, execute/verify each held-out entry; a failing entry is a blocking FAIL whose
  report names the failed entry — the burned entry is thereby revealed, which is the
  feedback loop working — while unrevealed entries stay unrevealed: the report never
  enumerates the set nor reveals entries that passed.
