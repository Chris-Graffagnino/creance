...
- [ ] T801 [strong] Runtime-neutral maker-eval workflow doc: a small frozen corpus
      **seeded from real Creance signals** (retrospective escapes, discovered-work
      ...
      judge report, first-upstream-failure class), the **triple fingerprint**
      (maker-behavior + judge identity + eval instrument), the observe-only boundary
      (P5), and corpus/rubric/judge-prompt/calibration-set-via-PR-only (P4); add the
...
- [ ] T802 [strong] Eval-run record emission (append-only JSONL, one per corpus
      ...
      report, compact first-upstream-failure class) + **triple fingerprint capture**
      (maker-behavior = model resolution + instruction/runtime surfaces; pinned-judge
      identity; eval instrument = corpus/prompts/rubrics/judge-prompt/scoring-schema);
...
- [ ] T806 [strong] Judge calibration artifact + agreement reporting: a small
      **owner-labeled calibration set** (maker outputs paired with the owner's
      ...
      figure (P5) and defines the agreement floor triage reads (T803). The maker
