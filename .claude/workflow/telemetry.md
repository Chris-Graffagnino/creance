...
| `fail_reports` | for every FAIL verdict in `rounds`: the reviewer's verdict report text **verbatim**, keyed by auditor and round (a NO-RESULT dispatch has no report — record the literal string `NO-RESULT` for it) — this is what the risk-ranked PR digest later cites for near-misses. A round aborted because the provided committed diff failed its checked, fail-closed contract (`gate-loop.md` → "Constraints inherited") records the classification the same way, keyed `diff-provider` and round — no reviewer was dispatched that round, so no reviewer is blamed for it. A run aborted **before round 1** because a rostered reviewer **[role]** could not be resolved in the dispatch context (`gate-loop.md` → "Inputs") records its diagnostic the same way, keyed `preflight` — again no reviewer was dispatched, so none is blamed |
| `commit` | full SHA of the **head commit of the diff the gate audited** (the task-branch head versus the base branch). Stamped by the dispatcher at append time, like the envelope — the loop's runtime has no filesystem to read it. It pins the record to the *exact introducing diff*, so the retrospective's Fact B attribution is **deterministic** rather than timing-correlated (`retrospective.md` §4). On an **abort record** — a `diff-provider` or `preflight` abort (`fail_reports` names the aborting step and `rounds` is **empty**, because no reviewer graded a diff) — this is the head that *would* have been audited, **not** a graded diff; the empty `rounds` is the disambiguator, so Fact B (which seeks a gate that actually **ran and passed** on the diff) never reads an abort record's `commit` as a graded-diff attribution. |
