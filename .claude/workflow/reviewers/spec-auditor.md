...
themselves*. In addition to everything above, screen **each drafted criterion** against
`workflow/intake.md` §4:
...
- **Not trivially gameable** — apply §4's **gameability screen**: name the cheapest way to
  ...
  is one-sided or trivially satisfiable. A criterion that is checkable but gameable is a
  **FAIL**, not a pass; the minimal fix is the tightening that penalizes both failure
  directions (per §4's worked examples).

This is a criterion-design screen layered on the normal review; it changes no gate semantics
(roster, dispatch, round limits, tier floors unchanged).
