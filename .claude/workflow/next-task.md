...
- If the user named a task ID, use it. Otherwise read the profile's **task index** — a
  generated digest of each task's selection-critical fields (path per the profile) — and pick
  the **lowest-numbered unchecked task whose dependencies are met**, then load that task's full context (§2). Task-ID format per the profile.
