...
| **[backlog-loop]** *(optional)* | An engaged autonomous run's activation state (consulted, never assumed — see `backlog-loop.md`), an **iteration budget N** resolved from configuration/invocation (never hardcoded; N=0 is a valid no-op), and the loop spec (`backlog-loop.md`) | A bounded sequence of **complete, unmodified** `next-task.md` cycles — each ending at a §7-gated PR (promote) or a discard — plus one observe-only run-report line per iteration and a terminal stop-condition summary | **Optional** — an adapter without it runs one task per session, losing only the unattended chaining. Reachable **only** through the **[autonomy activation]** decision of `next-task.md` §0.5 (off by default, fails **closed** to review — the loop never starts, and never falls back to un-isolated iterations). Stop conditions are **deterministic** and evaluated only *between* iterations, never mid-task (`backlog-loop.md` → "Stop conditions"). Composes existing roles verbatim — **no new gate semantics** (round limits, veto authority, tier floors, reviewer roster unchanged). **Never merges, never writes the base branch:** promotion inside an iteration is the §7 gate's PASS→PR path; merge stays session-explicit. The run report is observe-only telemetry under `telemetry.md`'s law — nothing reads it back into a gate, tier, or selection decision (P5) |
...
dimensions 6–7 — test adequacy, maintainability and clarity — graded against craft
practice rather than the fixed rubric; **whether and where it runs is the profile's
review-pass set** (above), not a fixed pairing with the [code-review pass].)
...
- `backlog-loop.md` — the **[backlog-loop]** spec: bounded unattended chaining of
  complete, unmodified `next-task.md` cycles (deterministic between-iteration stop
  conditions, PASS→PR / FAIL→discard per iteration, observe-only run report); reachable
  only through the [autonomy activation] check, and composing existing roles with no new
  gate semantics.
