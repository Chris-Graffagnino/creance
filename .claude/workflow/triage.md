...
- **Maker eval (from the maker-eval records).** The maker analog of "Gate trends" — a
  ...
  - **Trajectory differentials (per-interval scores — versioned; spec 003 US3.AC3).** A
    ...
    measurement"). Difference two runs' per-interval scores **only when the pair is
    comparable under the JUDGE-CHANGED / INSTRUMENT-CHANGED rule above — matching
    judge-identity and eval-instrument fingerprint components — and both records carry
    ...
    **INSTRUMENT-CHANGED / not-comparable** for the trajectory comparison — the trajectory
    ...
    still gets differenced violates the contract). The version alone is **not** the gate: a
    cadence-only instrument change moves eval-instrument while the trajectory version stays
    put, and a judge swap moves judge-identity — either pair's trajectories are just as
    confounded, so a fingerprint-moved pair **suppresses the trajectory differential** the
    same way it suppresses the regression call (rendered with the same JUDGE-CHANGED /
    INSTRUMENT-CHANGED annotation). A record with no trajectory renders the
...
- Trajectory: <one of: "comparable — trajectory instrument version <v> on both runs, pair fingerprint-comparable"  |  "INSTRUMENT-CHANGED / not-comparable — trajectory instrument versions differ (<v1> vs <v2>); trajectory differential suppressed"  |  "JUDGE-CHANGED / INSTRUMENT-CHANGED / not-comparable — judge or eval-instrument fingerprint moved between the two runs; trajectory differential suppressed"  |  "no trajectory recorded">
