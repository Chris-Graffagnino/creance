...
| `adapters/runtime-context-floor.py` | KEEP | Claude Code adapter command for aggregate-only runtime-attached context measurement and authored+floor reporting (T1207). Verbatim; it fails open on an unknown inventory shape and never gates. |
| `adapters/runtime-context-floor.test.sh` | KEEP | Fidelity, privacy, fail-open, baseline/report, P1/P5-boundary, and CI-wiring tests for the runtime floor command. Verbatim. |
| `adapters/runtime-context-floor-baseline.json` | GENERICIZE | Named non-gating fresh-session floor generated by the adapter command. Regenerate from the adopter runtime's fresh-session inventory; never carry Creance's measured value as the adopter's baseline. |
...
| `hooks/backlog-loop-select.test.sh` | KEEP | Selection-binding tests over fixture tasks trees — every documented selection rule, including blocked-bullet vs prose-mention discrimination and met/unmet `Blocked by` ranges. Verbatim. |
...
| `hooks/maker-eval-emit.sh` | KEEP | Adapter-side maker-eval record + transcript-packet emitter, triple-fingerprint capture, and interval trajectory-snapshot capture (`snapshot-run`, T807 — silent-to-the-run, instrument-declared cadence, explicit trajectory-incomplete marking). Observe-only. Verbatim. |
| `hooks/maker-eval-emit.test.sh` | KEEP | Behavioral tests for the maker-eval emitter — fingerprint independence, record/packet fencing, silent-write, partial-run completeness, and the T807 snapshot cases (cadence fixture, byte-identical-on-write-failure, two-sided trajectory-incomplete marking, fixed-boundary scheduling under slow copies, in-flight-child reaping). Verbatim. |
...
| `hooks/write-intents-check.sh` | KEEP | Deterministic write-intent / safe-output contract check (T636/#232, hardened by #284) — the contract's closed family stays non-empty; declarations are in-family; adapter-table catalog/discovery agree and every table maps the full family; every writing ritual has exactly one declaration row; no neutral `workflow/**` doc names a concrete tracker write command. Verbatim. |
...
| `workflow/reviewers/maker-eval-corpus.md` | KEEP | Frozen instrument manifest for maker-eval — corpus, per-dimension lifecycle metadata, rubrics, pinned-judge spec, scoring schema, calibration pointer, snapshot cadence. Portable; verbatim. |
...
4. **GENERICIZE** `README.md`, `settings.json`, `MODELS.md`, `context-budgets.md`,
   `adapters/runtime-context-floor-baseline.json`
