...
| `hooks/maker-eval-fence.sh` | KEEP | Deterministic P5 fence — asserts the maker-eval channel (records.jsonl + packets/) is referenced only by the eval writer, its run binding (`skills/maker-eval/SKILL.md`), and the triage reader (plus the non-control declaration/probe-doc/test surface), never a gate/tier/guard/selection path. Verbatim. |
