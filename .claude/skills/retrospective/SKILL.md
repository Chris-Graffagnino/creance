...
The auditors do two things: grade the committed diff they're handed, **and read the surrounding
...
`spec-auditor.md` all say so). Handing over a diff range alone fixes the first but not the second:
...
is the change *as written then*. The reviewer bindings grant **no shell** (#188), so **you** run
`git -C <tmp> diff <parent>..<introducing-commit>` and pass that diff in each auditor's prompt —
**not** `git diff main..HEAD`, which would capture the live tree against today's `main` (the wrong
range and the wrong tree for a back-test) — and instruct each to read surrounding context with
Read/Grep/Glob **only within `<tmp>`, never the live repo root** (a live read would grade the wrong
tree). For a squash-merged PR the
introducing commit is its squash commit on the base (tree = the change applied to its merge
base); for a regular commit it is the commit itself. `git worktree remove <tmp>` when done. This
stays read-only — a detached worktree and a ranged `git diff` (run by you, the dispatcher) mutate
no branch and no tracked state.
