...
| **interval snapshot capture** (the trajectory measurement — T807, US3.AC1) | wrap each maker [headless run] in the writer's capture driver: `bash .claude/hooks/maker-eval-emit.sh snapshot-run --run-id <id> --task <ME-id> --tier <maker-tier> --workspace <dir> -- claude -p "<the task's materialized prompt>" --model <the tier's row>` — it runs the maker command untouched (stdout/stderr pass through, exit code propagated exactly) while capturing workspace snapshots once per elapsed interval of the **instrument-declared cadence** (`reviewers/maker-eval-corpus.md` → "Snapshot cadence" — a frozen, fingerprinted value) into the channel's own trajectory storage. **Silent-to-the-run:** a failed or partial capture never blocks, fails, or alters the maker's run, and **no snapshot-derived value reaches the maker or this binding's output** (grading is post-hoc — US3.AC3, the next task's). A run spanning ≥ 1 declared interval that captured zero snapshots is recorded **trajectory-incomplete** explicitly by the writer, never silently. This DRIVES the writer (a sanctioned append), never reads the channel (the P5 fence line-scope) |
| **post-hoc snapshot grading + the versioned trajectory extension** (T808, US3.AC3) | after a task's maker run (and before its `record` call), drive the writer's grading driver: `bash .claude/hooks/maker-eval-emit.sh grade-snapshots --run-id <id> --task <ME-id> --tier <maker-tier> --out <scratch>/traj.json -- <judge-cmd...>` — the fence-trusted **writer** enumerates that (task × tier)'s captured interval snapshots internally and runs the judge command **once per snapshot** with the snapshot's directory appended as the final argument. The judge command is a **[headless run] of the pinned judge** (the `maker-eval judge` row of `.claude/MODELS.md`, never a maker tier row): `claude -p "<the task's rubric-grading prompt for one snapshot>" --model <the judge row>` — the **same per-task rubric and scoring schema** as the endpoint, one grading pass per snapshot, no fix loop. Then pass `--trajectory <scratch>/traj.json` on that (task × tier)'s `record` call (the emission row above), landing the per-interval scores **in the same record** under the **instrument-declared trajectory version** (`reviewers/maker-eval-corpus.md` → "Trajectory grading" — frozen, fingerprinted, P4; the emitter refuses a version-less or malformed trajectory loudly). A zero-snapshot slot yields `{ intervals: [] }` — **omit** `--trajectory` then. This binding **never resolves the channel or reads the snapshot storage itself**: the channel read happens inside the writer, whose output exists only to be appended back — a sanctioned write-pipeline drive under the P5 fence line-scope, unlike the surfacing read the triage reader owns |
...
fresh eval. Because a **complete** run has re-scored every corpus task at **every** maker tier
("the maker" above), clearing the flag certifies *all* tiers the fingerprint spans, not just one —
a single-tier run never completes, so it can never clear it. That flag is the on-change trigger to invoke this skill; the weekly schedule is the
