...
| **[isolated workspace]** (the §4 enter / end-of-run exit) | the worktree lifecycle in `.claude/hooks/isolated-workspace.sh`: `enter <branch> [--base <ref>]` adds an ephemeral git worktree on a **fresh** branch and **prints its path** on stdout — capture it and carry it forward (explicit-context); `exit <path>` tears the workspace directory down (the branch is left for the gate to promote/discard). `enter` **fails loud** (non-zero, no path) → **abort** the autonomous run, never fall back to the base branch. Ships **lifecycle only**: the §7 gate reading the workspace diff and the promote-on-PASS / discard-on-FAIL path are **T612**, so today an isolated run still terminates at the review-mode PR — nothing reaches the base branch outside the §7-gated human path. T611 / DESIGN-NOTES §14 |
