...
| **[edit guard]** | A policy on the edit tools (`sys_os_edit` / `sys_os_write`) that reruns the profile's configured checker on the touched file and returns `DENY` (fix-forward feedback) when the edit raises diagnostics **above the file's committed baseline**, allowing an edit no worse than before; **fails open** when no checker maps to the type. **Implemented + unit-tested at T618.** **Phase caveat (discovered at T618):** `docs/POLICIES.md` documents the `tool_call` (pre-write) and `request` phases but **no post-write phase**, so the README's original `tool_result` binding is **UNVERIFIED**. The delta is measured against the committed (HEAD) baseline, so it is only correct **post-write**; a *pre-write* firing would lint the file's current (pre-edit) state and could DENY the very edit meant to fix an already-dirty file (PR #137 review — Codex P2 / craft H1). So the port fires **only on a post-write phase** (`result_phases`, default `tool_result`) and **abstains on every pre-write phase** — never a false reject; until T620 pins the real post-write phase it abstains on all *documented* phases (inert-but-correct, like the reference `guard.sh` running PostToolUse only). The exact firing phase + the edit-argument key carrying the path are pinned on the live driver at **T620** |
...
- **Phase caveat (the `tool_result` question — discovered + handled at T618):** the README's
  ...
  documents only `tool_call` (pre-write) and `request` phases. Because the check compares the
  touched file's current diagnostics against its **committed (HEAD) baseline**, it is only
  correct on a **post-write** firing; a pre-write firing would read the file's pre-edit state
  and could DENY a fix-forward edit to an already-dirty file (PR #137 review). The T618 port
  therefore fires **only on a post-write phase** (`result_phases`, default `tool_result`) and
  **abstains on every pre-write phase** (fail-open, never a false reject), recording the exact
  firing phase as UNVERIFIED → pinned at T620; see the `[edit guard]` row above.
- **Telemetry caveat (deferred to T620 — PR #137 review):** `workflow/telemetry.md` makes the
  `[guard]` the emitter of `block` records (one per DENY) and the per-gate `evaluation`
  liveness record. The T618 port emits **neither** — emission needs the live telemetry-stream
  path (`PROJECT.md` → "Paths" → Telemetry) and a real dispatch context, and the `evaluation`
  signal is only meaningful once a gate actually runs on a driver: both are **T620**
  (real-driver liveness). Per telemetry.md's law ("telemetry observes; it never decides") a
  silent stream changes no DENY/abstain decision, so the deferral is behaviour-preserving.
...
| [guard] telemetry | `block` + per-gate `evaluation` liveness records (`telemetry.md`) not emitted by the port; needs the live stream path + a real gate run — pinned at T620. Telemetry never gates, so behaviour-preserving |
| [edit guard] firing phase | No post-write phase in `docs/POLICIES.md` (only `tool_call`/`request`); fires only on a post-write phase (`result_phases`, default `tool_result`), abstains on every pre-write phase (never a false reject); exact phase + edit-arg path key pinned at T620 |
