...
This is **T617 (T616 epic · part a)** — the skeleton: the role→mechanism table (`#119` AC1)
and the deterministic neutral-core-untouched check (`#119` AC4). The mechanism
*implementations* land in later sub-tasks and are **described here, not built here**:
the `[guard]` policies (`creance_omnigent/policies/guard.py`) at **T618** (AC2), the
cross-vendor `[reviewer]` sub-agents (`reviewers/*.yaml`) at **T619** (AC3), and the
orchestrator `config.yaml` + conformance probes + live-driver run at **T620** (AC5 +
Done-when). Mechanism facts below were verified against Omnigent's docs (sources at the
end, fetched 2026-06-23); anything not verifiable is marked **UNVERIFIED** rather than
asserted.
...
| **[guard]** | A deterministic **`tool_call` policy** — `type: function`, `handler:` a dotted path into `creance_omnigent/policies/guard.py`, discovered via `policy_modules:` + a `POLICY_REGISTRY` export, attachable server-wide / per-agent / per-session. The policy receives `event["type"] == "tool_call"` with `target` (e.g. `sys_os_shell`, `sys_os_edit`) + `data.arguments`, and returns `{"result": "DENY", "reason": …}` on a banned action, abstaining (`None`) / `ALLOW` otherwise. Several rules **delegate to builtins**: `omnigent.policies.builtins.github.github_policy` (write-branch allowlist = no commit/push to base; `shell_tools: ["sys_os_shell"]`), `…builtins.block_working_dir_changes` (blocks `git -C` / worktree escapes), `…builtins.cost.cost_budget` (a spend cap the harness currently lacks). **Fail-open is made explicit in the port** (the framework's on-error default is **UNDOCUMENTED / UNVERIFIED** — see "The [guard] binding"). Rules implemented + unit-tested at **T618** |
| **[edit guard]** | A **`tool_result` policy** on the edit tools (`sys_os_edit` / `sys_os_write`) that reruns the profile's configured checker on the touched file and returns `DENY` (fix-forward feedback) when the edit raises diagnostics **above the file's committed baseline**, allowing an edit no worse than before; **fails open** when no checker maps to the type. Omnigent fires policies on the edit-tool events (so — unlike the Codex `apply_patch` gap — edit-boundary enforcement is reachable; `tool_result` timing makes it fix-forward, not a pre-write veto). Implemented at **T618** |
...
- **Caveat (UNVERIFIED → resolved at T618/T620):** `docs/POLICIES.md` does **not** document
  whether a policy that *raises* is treated as fail-open or fail-closed. The `[guard]`
  contract mandates **fail-open**, so the port wraps every rule so any internal
  exception/uncertainty returns `None`/`ALLOW`, and a T618 unit test asserts it. The
...
| [guard] on-error posture | Framework fail-open/closed default UNVERIFIED; the port forces fail-open + a T618 test |
