...
# A task's own issue/PR link, recorded as a trailing `[#NNN]` marker on its (joined) line
# (US5.AC1 'when known'), optionally before the story tag. Deliberately strict — a trailing
# marker only — so the ambiguous inline `(#NNN)` provenance prose (e.g. `(#103, surfaced by
# … #80)`, or a `#NNN` that recurs across tasks) is never mis-read as the link.
ISSUE_RE = re.compile(r"\[#(\d+)\]\s*(?:\(US\d+\)\s*)?$")
...
def issue_link(desc):
    """A task's issue/PR reference when its tasks-file line records one as a trailing
    `[#NNN]` marker (US5.AC1 'when known'); empty otherwise. The current backlog records
    provenance as free prose and adopts no such marker, so every row is honestly blank —
    the reference is never guessed from the ambiguous inline `(#NNN)` prose (see ISSUE_RE)."""
    m = ISSUE_RE.search(desc)
    return "#%s" % m.group(1) if m else ""


def short_title(desc):
    """A compact, deterministic title: the description up to its first em-dash or
    semicolon (authors put the gist first), trailing `(US#)` tag and `[#NNN]` link marker
    dropped, capped at a word boundary. Selection-critical, not the whole task — full
    context loads later (US5.AC3)."""
...
def build_index():
    ...
    for path in files:
        ...
        if not tasks:
            continue
        rows = [
            (tid, tier, state, criteria_field(tid, ownership, desc),
             issue_link(desc), short_title(desc))
    ...
    for sp, rows in sections:
        ...
        for tid, tier, state, crits, issue, title in rows:
            lines.append(
                "| %s | %s | [%s] | %s | %s | %s |" % (tid, tier, state, crits, issue, title)
            )
    return "\n".join(lines) + "\n"
