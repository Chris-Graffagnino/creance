...
| Reviewers enforce maker≠checker — they grade but never apply their own fixes (P4) | constitution-auditor: a reviewer binding granting a file-editing tool, or a gate/§7 path that lets a reviewer apply/commit its own finding instead of returning a verdict for a separate fix step | `reviewer-roster.test.sh` AC5 (every reviewer agent excludes edit tools) + `gate-loop.test.js` (reviewers dispatched verdict-only; a separate fix step, the maker role, owns edits). **Known gap:** reviewers also grant `Bash`, an unenforced shell-write vector — read-only Bash is a *behavioral contract* spot-checked by the P-RV mutation lure, not deterministically blocked; hardening tracked in #188 |
